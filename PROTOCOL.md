...
Firmware detects the known "Taste the Rainbow" signature (bytes 2–13 of the payload) and renders a discrete 6-color-band sweep instead — matching the real band's actual look (confirmed live: a smooth continuous HSV hue blend, tried first, did not match; discrete color jumps, adapted from the Adafruit reference renderer's own approach for this exact signature, did). Any other `E9 0C` payload (5-Palette-Color-Cycle, etc.) falls back to a generic multi-slot palette decode + rotation instead.

**`Blink White` and `Orange Blink` are also detected by exact signature** (`Blink White` shares "Taste the Rainbow"'s 12-byte prefix, disambiguated by the payload's last byte: `0x95`=Blink White, anything else including the real `0xB0`=Rainbow). Both reuse the existing `E9 0E` strobe renderer with a different color rather than new render logic. Confirmed live: the real band's **Blink White is a much slower ~0.5s on/off blink**, not `E9 0E`'s fast strobe rate — firmware sets a distinct, slower blink interval for this specific case. `Orange Blink`'s real captured Timing Byte has `ALWAYS_ON` set (an alert-style animation meant to hold indefinitely on the real band) — firmware deliberately overrides this to a fixed 5s duration instead, since indefinite hold isn't useful for a wearable prop.
